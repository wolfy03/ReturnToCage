...
func run(t: Node) -> void:
	...
	t.assert_true(not GameSession.handle_player_death(Vector2.ZERO).success, "menu death rejected explicitly")
	...
	t.assert_true(not GameSession.handle_player_death(Vector2.ZERO).success, "missing start content returns failure instead of dereference")
	...
	var life: int = GameSession.arm_player_life()
	...
	var result: RespawnResult = GameSession.handle_player_death(Vector2.ZERO, life)
	...
	t.assert_true(GameSession.handle_player_death(Vector2.ONE, life) == result, "duplicate death returns cached result")
	...
	var next_life: int = GameSession.arm_player_life()
	t.assert_true(next_life != life and GameSession.last_death_result == null, "new actor advances life and clears result")
	before = GameSession.export_state()
	t.assert_true(not GameSession.handle_player_death(Vector2.ZERO, life).success, "late retired life death ignored")
	...
	old_actor.free()
	GameSession.handle_player_death(Vector2.ZERO)
