...
const PLAYER_ONE: StringName = &"player_11111111111111111111111111111111"
...
func run(t: Node) -> void:
	...
	NetworkManager._set_identity(43, PLAYER_THREE)
	var peer_b := GameSession.attach_player(42, PLAYER_TWO)
	var peer_c := GameSession.attach_player(43, PLAYER_THREE)
	...
	t.assert_true(GameSession.get_player_state_by_player_id(PLAYER_TWO) == null and GameSession.get_player_state_by_player_id(PLAYER_THREE) == null, "session end removes remote persistent registry entries")
