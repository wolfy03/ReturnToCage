...
func run(t: Node) -> void:
	t.assert_equal(NetworkProtocol.VERSION, 12, "world-scoped gameplay replication uses protocol v12")
	...
	t.assert_equal(NetworkSessionSnapshot.from_payload(payload, NetworkProtocol.VERSION, NetworkManager.MAX_PLAYERS).error_message, "Incompatible multiplayer protocol version", "v11 session snapshot is rejected by protocol v12")
