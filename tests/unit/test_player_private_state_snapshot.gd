...
func _test_client_sync_gate(t: Node) -> void:
	...
	t.assert_true(NetworkManager._received_spawn_assignment and not NetworkManager._session_entered, "spawn assignment still waits for the owner world assignment")
	var world := PlayerWorldAssignment.from_world_state(
		"private-sync-test", PlayerWorldState.settlement(local_id)
	)
	NetworkManager._receive_world_assignment(world.to_payload())
	t.assert_true(NetworkManager._received_world_assignment and NetworkManager._session_entered, "world assignment completes the client synchronization gate")
	t.assert_equal(synchronized[0], 1, "session_synchronized emits once after all four snapshots")
	NetworkManager._receive_private_player_state(private_snapshot.to_payload())
