...
func _on_damaged(context: DamageContext) -> void:
	if not is_simulation_authority() or _death_resolved:
		return
	apply_external_impulse(context.knockback)
...
## Enemy equivalent of MovementComponent's authoritative additive impulse.
func apply_external_impulse(impulse: Vector2) -> bool:
	if not is_finite(impulse.x) or not is_finite(impulse.y):
		return false
	velocity += impulse
	return true
