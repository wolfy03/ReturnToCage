...
## attack timeline, and MovementComponent owns locomotion/physics. This component
## coordinates those boundaries without owning damage or knockback.
...
func begin_hurt() -> bool:
	...
	if action.is_hurt():
		...
		movement.set_controls_locked(true)
		return true
...
func physics_tick(delta: float) -> void:
	...
	if action.finish_hurt():
		movement.set_controls_locked(false)
