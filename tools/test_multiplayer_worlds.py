...
SCENE = "res://tests/integration/multiplayer_world_probe.tscn"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run split-world ENet processes.")
    parser.add_argument("--godot", default="godot")
    parser.add_argument("--players", type=int, choices=(2, 3), default=2)
    parser.add_argument("--port", type=int, default=0, help="Use 0 to allocate a free localhost port.")
    ...
    with tempfile.TemporaryDirectory(prefix="return_to_cage_worlds_") as temporary:
        try:
            ...
            launch("host", "host", 0)
            wait_for_marker(processes[0][1], processes[0][2], "WORLD PROBE HOST LISTENING", 10.0)
            ...
            for name, process, log_path, log_file in processes:
                ...
                print(f"[{name}]\n{output}")
                if process.returncode != 0 or "WORLD PARTICIPATION PROBE PASS" not in output \
            ...
            print(f"split-world probe failed: {error}")
            return 1
        finally:
