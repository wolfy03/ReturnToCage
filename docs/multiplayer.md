...
Protocol v10 adds a separate owner-only `PlayerPrivateStateSnapshot` containing `player_id`, base stats, survival state, non-equipment active effects, and last-safe position. The server derives its target from the accepted sender mapping and sends the payload only with `rpc_id(owner_peer_id)`; clients cannot request an arbitrary identity. The client validates the complete payload against its profile and attached local `player_id`, stages all replacement models, then atomically replaces those private domains. Session metadata, private state, and a session/owner-bound authoritative spawn assignment must all apply before `session_synchronized` opens the world. The Settlement scene consumes that cached assignment while it is created, before the local actor becomes world-ready. Inventory/equipment and PERSONAL quests retain their existing owner-only revisioned services, health/life retains its runtime snapshot, and shared domains remain outside this DTO. Reapplying a snapshot replaces effects and derived modifiers instead of accumulating them.
