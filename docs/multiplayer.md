...
- Protocol v10 owner-private synchronization for base stats, survival, active effects, and last-safe position before client session readiness
...
Protocol v10 adds a separate owner-only `PlayerPrivateStateSnapshot` containing `player_id`, base stats, survival state, non-equipment active effects, and last-safe position. The server derives its target from the accepted sender mapping and sends the payload only with `rpc_id(owner_peer_id)`; clients cannot request an arbitrary identity. The client validates the complete payload against its profile and attached local `player_id`, stages all replacement models, then atomically replaces those private domains. Session metadata and private state must both apply before `session_synchronized` opens the world. Inventory/equipment and PERSONAL quests retain their existing owner-only revisioned services, health/life retains its runtime snapshot, and shared domains remain outside this DTO. Reapplying a snapshot replaces effects and derived modifiers instead of accumulating them. Last-safe position is synchronized here but is not yet used as a spawn policy.
...
Known P2/deferred work is spawn and end-to-end process-restart orchestration. Protocol v10 now mirrors owner-private base stats, survival, effects, and last-safe position, but using the safe position for a returning player's spawn and invalid-position fallback is intentionally deferred to 6/7. Full new-process 2/3-player reconnect probes remain deferred to 7/7.
