...
The read-only identity recovery backend can inspect deterministic candidate IDs from the primary Save v4 and can validate an explicitly selected candidate through the complete detached staging path before requesting identity commit through `NetworkManager`. `LocalPlayerProfile` remains the profile-file persistence implementation but is not exposed as a mutable general service. Recovery does not auto-select, apply the staged game session, or use legacy saves or the game-save `.bak` as identity sources. A successful profile commit and the later explicit offline `GameSession` identity activation are separate lifecycle steps.
