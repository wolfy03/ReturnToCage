...
회피는 서 있는 상태(지상)에서 `IDLE`일 때만 시작한다. 공중·등반 중이거나 공격·피격 경직 중에는
시작할 수 없고, 반대로 회피 중에는 공격·상호작용·아이템 사용·귀환 채널링이 모두 막힌다. 무적은
회피 전체가 아니라 앞부분 구간에만 적용되므로 후반부는 그대로 맞는다. 스태미나는 시작 시 한 번
빠지고 벽에 막히거나 낭떠러지로 떨어지거나 도중에 피격돼도 돌려받지 않는다. 회피 중 낭떠러지를
벗어나면 그대로 낙하하며, 벽은 통과하지 못한다. 굶주림·탈수 피해는 무적으로 막을 수 없다.
